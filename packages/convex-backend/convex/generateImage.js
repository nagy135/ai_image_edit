...
export const generateNextStep = action({
    args: {
        ...
        prompt: v.string(),
        editType: v.union(v.literal("align_left"), v.literal("align_right"), v.literal("center"), v.literal("make_old"), v.literal("make_young"), v.literal("delete_background"), v.literal("add_background"), v.literal("remove_object"), v.literal("make_square"), v.literal("make_circular"), v.literal("duplicate_object"), v.literal("manual"), v.literal("zoom"), v.literal("brightness"), v.literal("unknown")),
    ...
    handler: async (ctx, args) => {
        ...
        const latestImage = images.reduce((prev, cur) => cur.stepNumber >= prev.stepNumber ? cur : prev);
        ...
        const storedId = await ctx.storage.store(new Blob([arrayBuffer], { type: mimeType }));
        // Append step in DB
        await ctx.runMutation(internal.images.addImage, {
            ...
            brightnessPercent: args.brightnessPercent ?? primaryImage.brightnessPercent ?? 100,
        });
        return {
            stepNumber: nextStepNumber,
            storageId: storedId,
